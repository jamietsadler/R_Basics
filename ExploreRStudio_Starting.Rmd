...
temps_f <- c(95, 65, 78, 76, 89, 76, 87)
...
```{r}
# Apply your function to temp_f her
temps_c <- sapply(temps_f, convert_f_to_c)
...
over_60 <- function(temps_f){
  a <- length(temps_f[temps_f>60])
  return (a > length(temps_f)/2)
}

print(over_60(temps_f))
...
new_temps <- list(
  London <- c(23, 45, 56, 59, 87, 67, 87),
  Paris <- c(76, 56,45, 67, 78, 56 ,87)
)
...
library("ggplot2")
destination_df <- as.data.frame(new_temps)
